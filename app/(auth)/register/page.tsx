        ...
        <form onSubmit={handleVerify} className="space-y-4" noValidate>
          ...
          <div className="space-y-1.5">
            ...
            <Input id="code" type="text" placeholder="123456" maxLength={6}
              ...
              disabled={loading} className="text-lg tracking-[0.5em] text-center" required />
          </div>
  ...
  return (
    <div className="w-full max-w-sm space-y-6">
      <div className="space-y-1">
        ...
        <p className="text-sm text-muted-foreground">Start analyzing YouTube channels for free</p>
      </div>

      <div className="space-y-2.5">
        <button type="button" onClick={() => handleOAuth("oauth_google")} disabled={busy}
          className="w-full flex items-center justify-center gap-2.5 h-10 rounded-lg border border-border bg-background hover:bg-muted text-sm font-medium transition-colors disabled:opacity-50 cursor-pointer">
          {oauthLoading === "google" ? <Loader2 className="size-4 animate-spin" /> : <GoogleIcon />}
          Continue with Google
        </button>
        <button type="button" onClick={() => handleOAuth("oauth_github")} disabled={busy}
          className="w-full flex items-center justify-center gap-2.5 h-10 rounded-lg border border-border bg-background hover:bg-muted text-sm font-medium transition-colors disabled:opacity-50 cursor-pointer">
        ...
        <div className="grid grid-cols-2 gap-3">
          <div className="space-y-1.5">
            ...
            <Input id="firstName" placeholder="Jane" value={firstName}
              onChange={(e) => setFirstName(e.target.value)} disabled={busy} required />
          </div>
          <div className="space-y-1.5">
            <Label htmlFor="lastName">Last name</Label>
            <Input id="lastName" placeholder="Smith" value={lastName}
        ...
        <div className="space-y-1.5">
          ...
          <div className="relative">
            <Input id="password" type={showPassword ? "text" : "password"}
              placeholder="Min. 8 characters" autoComplete="new-password" className="pr-10"
